...
module.exports = {
     ...
     * @return {Array} Returns an array of file contents to be added to Quick Base OR false if any files were misssing.  Index 1 is filename, Index 2 is file contents to be added to Quick Base, and if the file is the main launch file (isIndexFile = true in qbcli.json) return true at index position 3 in the array 
     */
    getAllFileContents( filesArray, getFileContents, replaceAll, prefix ) {
        ...
        var contents = filesArray.map((item)=>{
            ...
            if( isIndexFile ) {
                return [filename, fileContents.replaceAll("]]>", "]]]]><![CDATA[>"), isIndexFile];
            } else {
     ...
     */
    prefixGenerator(customPrefix, customPrefixProduction, isProd, repositoryId) {
        //for dev
        if (!isProd) {
            if( customPrefix ) {
